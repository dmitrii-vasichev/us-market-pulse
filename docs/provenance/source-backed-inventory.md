...
| `overview.bullet-targets` | Overview / grid card 5 | `BulletTargets` | `/api/v1/kpi/summary` | `Source: BEA, BLS, Federal Reserve · Mar 10, 2026` | KPI summary derived from stored `GDP`, `CPIAUCSL`, `UNRATE`, and `FEDFUNDS`, with bullet measures, target bands, and markers attached by backend policy definitions | `economic_series` + `series_metadata` for KPI series; backend policy mapping in `backend/app/services/kpi_targets.py` | `derived` | Phase 3 implementation completed in Task 6; runtime now matches the approved backend-owned KPI policy contract | The chart remains derived because stored KPI observations are still interpreted against backend policy thresholds, but those measures, markers, and policy notes are now emitted by the API instead of living in frontend literals. |
